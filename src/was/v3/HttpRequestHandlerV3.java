...
public class HttpRequestHandlerV3 implements Runnable {
    ...
    public void run() {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream(), UTF_8));
            ...
            log("HTTP 응답 생성중...");
            if (requestToString.contains("GET / ")) {
                home(writer);
            } else if (requestToString.contains("GET /site1")) {
                site1(writer);
            } else if (requestToString.contains("GET /site2")) {
                site2(writer);
            } else if (requestToString.contains("GET /search")) {
